...
namespace ElasticScaleStarterKit
{
    internal class CreateShardSample
    {
        ...
        /// </summary>
        private const string InitializeShardScriptFile = "InitializeShard_WoT.sql";

        /// <summary>
        /// Format to use for creating shard name. {0} is the number of shards that have already been created.
        /// </summary>
        private const string ShardNameFormat = "ElasticScale_Shard{0}";
        ...
        /// </summary>
        public static Shard CreateOrGetEmptyShard(RangeShardMap<int> shardMap)
        {
        ...
        /// Finds an existing empty shard, or returns null if none exist.
        /// </summary>
        public static IEnumerable<Shard> FindEmptyShards(RangeShardMap<int> shardMap)
        {
            // Get all shards in the shard map
            IEnumerable<Shard> allShards = shardMap.GetShards();
            ...
            // Get the first shard (ordered by name) that has no mappings, if it exists
            return allShards.OrderBy(s => s.Location.Database).Where(s => !shardsWithMappings.Contains(s));
        }
    }
}
